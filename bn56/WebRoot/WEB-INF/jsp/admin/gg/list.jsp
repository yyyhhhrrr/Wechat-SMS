...
<html>
		...
		<display:table name="list"
			...
			<display:column title="查看" style="width:40px;">
				<img src="${ctx}/resource/admin/images/pencil.png"
					onclick="return modifyShow('${row.id}','show');" style="cursor:hand;"/>
			</display:column>
			<display:column title="标题" property="title"/>
		   
			
			<display:column title="日期">
				<fmt:formatDate value="${row.showDate}" type="date" pattern="yyyy-MM-dd"/>	</display:column>
						<c:choose>
  ...
  //var aaa = prompt("请输入操作意见或说明：","");
	...
	function modifyShow(id,method){
			MyWindow.OpenCenterWindow('show.html?id='+id+'&flag=${param.flag}&method='+method,'modify',800,800);
	}
