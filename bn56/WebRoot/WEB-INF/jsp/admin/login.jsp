...
jQuery(function($){
	...
	function loginCheck(){//对输入的用户名、密码判断
		...
		var aa="";//传入到注册界面控制器url赋值为aa
		var bb="";//传入到登录后主界面控制器url赋值为bb
		if(type=='教务人员'){
		...
		$.ajax({
		      ...
		      async:false,  // 设置同步方式
		      cache:false,
		      url: aa,//aa为之前定义的传输数据的控制器url
				data:$("#login_form").serializeArray(),//获取form表单输入的数据
				success:function(result){
				result = eval("("+result+")");
				if(result.status=='true'||result.status==true){
					if(result.msg=='1'){//判断控制器里返回的result值
						alert('登录成功');
